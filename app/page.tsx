...
import { 
  ...
  ShieldAlert, Trash2, AlertCircle, ZoomIn, ArrowRightLeft, Activity, Info
} from "lucide-react";
...
const analyzeWithRetry = async (publicUrl: string, settings: SettingsState, addToast: Function, maxRetries = 5) => {
  ...
  while (attempt < maxRetries) {
    try {
      ...
      if (analyzeRes.status === 429 || data.error?.includes("429") || data.error?.includes("Rate limit")) {
      ...
      const waitTime = err.message.includes("RateLimitExceeded") ? 30000 : 5000;
      addToast(`API Limit Hit! Auto retrying in ${waitTime / 1000}s... (${attempt}/${maxRetries})`, "error");
...
export default function NexVmetaPro() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    addToast(`${files.length} file(s) added to queue`, "info");
    ...
    for (const file of files) {
      ...
      setQueue(prev => [...prev, {
        id, name: file.name, preview: localPreview, status: "uploading", progress: 0, result: null
      }]);
      ...
      const progressInterval = setInterval(() => {
        ...
        setQueue(prev => prev.map(q => q.id === id ? { ...q, progress: progressVal } : q));
      }, 200);

      try {
        const { error } = await supabase.storage.from('nexvmeta-uploads').upload(fileName, file);
        clearInterval(progressInterval);
        if (error) throw error;

        const { data: { publicUrl } } = supabase.storage.from('nexvmeta-uploads').getPublicUrl(fileName);
        setQueue(prev => prev.map(q => q.id === id ? { ...q, status: "ready", progress: 100, publicUrl, preview: publicUrl } : q));
      } catch (err) {
        clearInterval(progressInterval);
  ...
  const runBatch = async () => {
    ...
    for (const item of pendingItems) {
      ...
      try {
        ...
        setQueue(prev => prev.map(q => q.id === item.id ? { ...q, status: "done", progress: 100, result: data } : q));
  ...
  const clearAll = () => {
    if(confirm("Are you sure you want to delete all data?")) {
      setQueue([]); localStorage.removeItem("nexvmeta_queue_v2"); setSelectedId(null);
  ...
  const exportAdobeCSV = () => {
    ...
    if (completedItems.length === 0) return alert("No completed files to export.");
    ...
    link.download = `AdobeStock_Batch_${new Date().toISOString().slice(0,10)}.csv`; link.click();
  ...
  return (
    <div className="h-screen bg-[#030308] text-white font-sans flex overflow-hidden selection:bg-blue-500/30 relative">
      ...
      <div className="fixed bottom-6 right-6 z-[100] flex flex-col gap-3 pointer-events-none">
        {toasts.map(t => (
          <div key={t.id} className={`flex items-center gap-3 px-5 py-3 rounded-2xl backdrop-blur-2xl border shadow-2xl animate-in slide-in-from-right-8 fade-in duration-300 pointer-events-auto ${t.type === 'success' ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-100' : t.type === 'error' ? 'bg-red-500/10 border-red-500/30 text-red-100' : 'bg-blue-500/10 border-blue-500/30 text-blue-100'}`}>
            {t.type === 'success' ? <CheckCircle size={18} className="text-emerald-400"/> : t.type === 'error' ? <AlertCircle size={18} className="text-red-400"/> : <Info size={18} className="text-blue-400"/>}
      ...
      <aside className="w-20 lg:w-64 bg-white/[0.02] backdrop-blur-3xl border-r border-white/5 flex flex-col z-20 shrink-0 shadow-2xl">
        <div className="p-6 border-b border-white/5 flex items-center gap-3">
           ...
           <div className="hidden w-9 h-9 bg-blue-600 rounded-xl flex items-center justify-center font-bold text-white shadow-[0_0_20px_rgba(37,99,235,0.4)]">N</div>
           
           <span className="font-bold text-xl tracking-tight hidden lg:block bg-linear-to-r from-white to-gray-400 bg-clip-text text-transparent">NexV<span className="text-blue-500">meta</span></span>
        </div>
                ...
                <div className="flex bg-black/40 rounded-xl p-1">
                   {["4K", "8K"].map(res => (
                      <button 
                        ...
                        className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${settings.resolution === res ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-500 hover:text-gray-300 hover:bg-white/5'}`}
                      >
                        {res}
                      </button>
                   ))}
              ...
              {[['Title', 'titleMin', 'titleMax', 5, 100], ['Description', 'descMin', 'descMax', 10, 300], ['Keywords', 'keywordMin', 'keywordMax', 5, 49]].map(([label, minKey, maxKey, minLimit, maxLimit]) => (
                <div key={label.toString()}>
                  ...
                  <div className="flex gap-2">
                     <input type="number" min={minLimit as number} value={settings[minKey as keyof SettingsState]} onChange={(e) => setSettings({...settings, [minKey]: parseInt(e.target.value) || 0})} className="w-full bg-black/40 border border-white/10 rounded-xl p-2 text-sm text-center focus:border-blue-500 outline-none backdrop-blur-md transition-all"/>
                     <span className="flex items-center text-gray-600">-</span>
                     <input type="number" max={maxLimit as number} value={settings[maxKey as keyof SettingsState]} onChange={(e) => setSettings({...settings, [maxKey]: parseInt(e.target.value) || 0})} className="w-full bg-black/40 border border-white/10 rounded-xl p-2 text-sm text-center focus:border-blue-500 outline-none backdrop-blur-md transition-all"/>
                  </div>
                </div>
              ))}
           </div>
    ...
    <div className="h-full flex relative z-10">
       <div className="w-80 bg-white/2 backdrop-blur-xl border-r border-white/5 flex flex-col relative shrink-0 shadow-2xl">
          <div className="p-4 flex gap-2 border-b border-white/5">
             ...
             <button onClick={() => fileInputRef.current?.click()} className="flex-1 py-4 border border-dashed border-white/10 hover:border-blue-500 rounded-2xl flex flex-col items-center gap-1 text-gray-400 hover:text-blue-400 transition-all bg-black/20 hover:bg-blue-500/10">
                <Upload size={20}/>
                <span className="text-[10px] font-bold uppercase tracking-wider">Upload</span>
             </button>
             <button onClick={clearAll} className="w-16 py-4 border border-dashed border-white/10 hover:border-red-500 rounded-2xl flex flex-col items-center gap-1 text-gray-400 hover:text-red-400 transition-all bg-black/20 hover:bg-red-500/10">
                <Trash2 size={20}/>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-2 space-y-2 custom-scrollbar">
             {queue.map((item: FileItem) => (
                <div key={item.id} onClick={() => setSelectedId(item.id)} className={`flex items-center gap-3 p-2.5 rounded-2xl cursor-pointer border relative overflow-hidden transition-all duration-300 ${selectedId === item.id ? 'border-blue-500/50 bg-blue-500/10 shadow-[0_0_20px_rgba(37,99,235,0.1)]' : 'border-white/5 bg-black/20 hover:bg-white/5'}`}>
                   {(item.status === 'uploading' || item.status === 'analyzing') && (
                     <div className={`absolute left-0 bottom-0 h-1 transition-all duration-300 ${item.status === 'analyzing' ? 'bg-linear-to-r from-purple-500 to-pink-500' : 'bg-linear-to-r from-blue-500 to-cyan-500'}`} style={{width: `${item.progress}%`}}></div>
                   )}
          ...
          <div className="p-4 border-t border-white/5 bg-black/20 backdrop-blur-md space-y-3">
             <button onClick={runBatch} disabled={processing || queue.filter((q: FileItem) => q.status === 'ready').length === 0} className="w-full bg-linear-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 disabled:opacity-50 disabled:grayscale text-white py-3.5 rounded-2xl font-bold text-sm flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(37,99,235,0.3)] transition-all">
               {processing ? <RefreshCw className="animate-spin" size={16}/> : <ScanEye size={16}/>} Start Cloud Analysis
             </button>
             <button onClick={exportAdobeCSV} disabled={queue.filter((q: FileItem) => q.status === 'done').length === 0} className="w-full bg-white/10 hover:bg-white/20 border border-white/10 disabled:opacity-50 disabled:cursor-not-allowed text-white py-3.5 rounded-2xl font-bold text-sm flex items-center justify-center gap-2 transition-all backdrop-blur-md">
             ...
             <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-500">
                <div className="flex gap-8 bg-white/5 border border-white/10 p-6 rounded-3xl backdrop-blur-2xl shadow-2xl">
                   <div className="relative group shrink-0">
                     <img src={selectedItem.preview} className="w-56 h-56 object-cover rounded-2xl shadow-2xl border border-white/10"/>
                     <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-4">
                        <span className="text-xs font-bold text-white uppercase tracking-widest">{selectedItem.name}</span>
                     </div>
                   </div>
                   <div className="flex-1 space-y-5">
                      <div>
                         <label className="text-xs text-blue-400 font-bold uppercase tracking-widest flex items-center gap-2"><Sparkles size={14}/> Optimized Title</label>
                ...
                <div className="grid grid-cols-2 gap-6">
                   <div className={`p-6 border rounded-3xl backdrop-blur-xl ${selectedItem.result.technical.quality_score > 80 ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-red-500/5 border-red-500/20'}`}>
                      ...
                      <div className="w-full bg-black/40 h-2 rounded-full mb-4 border border-white/5 overflow-hidden">
                         <div className={`h-full rounded-full transition-all duration-1000 ${selectedItem.result.technical.quality_score > 80 ? 'bg-gradient-to-r from-emerald-600 to-emerald-400' : 'bg-gradient-to-r from-red-600 to-orange-500'}`} style={{width: `${selectedItem.result.technical.quality_score}%`}}></div>
                      </div>
                   ...
                   <div className="p-6 bg-purple-500/5 border border-purple-500/20 rounded-3xl backdrop-blur-xl relative group">
                      <div className="text-xs text-purple-400 font-bold uppercase tracking-widest mb-3 flex justify-between items-center">
                         ...
                         <button onClick={() => {navigator.clipboard.writeText(selectedItem.result!.prompts.sanitized_prompt); addToast("Prompt copied!", "success")}} className="opacity-0 group-hover:opacity-100 transition-opacity bg-purple-500/20 p-1.5 rounded-lg hover:bg-purple-500/40 text-purple-300"><Save size={14}/></button>
                      </div>
                ...
                <div className="bg-white/5 p-6 rounded-3xl border border-white/10 backdrop-blur-xl">
                   ...
                   <div className="flex flex-wrap gap-2">
                      {selectedItem.result.meta.keywords.map((k: any,i: number) => (
                         <span key={i} className={`px-4 py-1.5 rounded-full text-xs font-medium border backdrop-blur-md transition-all hover:scale-105 cursor-default ${i < 10 ? 'bg-blue-500/10 border-blue-500/30 text-blue-200 shadow-[0_0_10px_rgba(37,99,235,0.2)]' : 'bg-white/5 border-white/10 text-gray-400'}`}>
                           {k.tag}
                         </span>
                      ))}
                   </div>
...
function BgRemoverView({ addToast }: { addToast: Function }) {
  ...
  const processBgRemoval = async () => {
    ...
    try {
      ...
      addToast("Failed: API Key Quota Exhausted.", "error"); 
    }
    setLoading(false);
  };
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center p-8 relative z-10">
       <div className="bg-white/[0.03] backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-10 max-w-2xl w-full text-center shadow-2xl">
          ...
          <p className="text-gray-400 mb-10 text-sm">Powered by Cloud AI</p>

          {!resultUrl ? (
            <div className="space-y-6">
              <input type="file" onChange={(e) => setImage(e.target.files?.[0] || null)} className="block w-full text-sm text-gray-400 file:mr-4 file:py-3 file:px-6 file:rounded-full file:border-0 file:text-sm file:font-bold file:bg-white/10 file:text-white hover:file:bg-white/20 file:backdrop-blur-md file:transition-all cursor-pointer bg-black/20 rounded-full p-2 border border-white/5"/>
              <button onClick={processBgRemoval} disabled={!image || loading} className="w-full bg-white text-black py-4 rounded-2xl font-bold disabled:opacity-50 hover:bg-gray-200 transition-all shadow-[0_0_20px_rgba(255,255,255,0.2)]">
            ...
            <div className="space-y-6 animate-in zoom-in-95 duration-500">
              <div className="relative p-2 bg-white/5 rounded-3xl border border-white/10">
                 <img src={resultUrl} className="max-h-72 mx-auto rounded-2xl bg-[url('https://media.istockphoto.com/id/1146311516/vector/checker-seamless-pattern-vector-transparent-grid-background-transparency-grid-texture.jpg?s=612x612&w=0&k=20&c=d5m6hT4fA0Q0A0o9y0_0A0')] bg-repeat shadow-inner"/>
              </div>
              <div className="flex gap-4">
                <button onClick={() => { setImage(null); setResultUrl(null); }} className="flex-1 py-4 border border-white/10 rounded-2xl hover:bg-white/10 font-bold transition-all backdrop-blur-md">Upload New</button>
                <a href={resultUrl} download="removed_bg.png" onClick={()=>addToast("Image downloaded!","success")} className="flex-1 py-4 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-2xl font-bold flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:scale-[1.02] transition-transform"><Download size={18}/> HD Download</a>
              </div>
            </div>
          )}
       </div>
...
function UpscalerView({ addToast }: { addToast: Function }) {
  ...
  const handleUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setImage(file); setPreviewUrl(URL.createObjectURL(file)); setResultUrl(null); setProgress(0);
    ...
    <div className="h-full flex flex-col lg:flex-row relative z-10">
          ...
          {!previewUrl ? (
             ...
             <div className="relative w-full h-full flex items-center justify-center bg-black/20 rounded-3xl border border-white/5 backdrop-blur-sm p-4">
                {viewMode === "compare" && resultUrl ? (
                  ...
                  <img src={resultUrl || previewUrl} className={`max-h-[85vh] object-contain rounded-2xl transition-all duration-700 ${resultUrl ? 'shadow-[0_0_60px_rgba(168,85,247,0.2)] border border-purple-500/20' : 'border border-white/5 opacity-70'}`} />
                )}
                  ...
                  <div className="absolute inset-0 bg-black/80 backdrop-blur-md flex flex-col items-center justify-center rounded-3xl z-10 animate-in fade-in duration-300">
                     <div className="w-72 space-y-6 text-center">
                        ...
                        <div className="w-full bg-white/10 h-2.5 rounded-full overflow-hidden border border-white/5">
                           <div className="h-full bg-linear-to-r from-blue-500 via-purple-500 to-pink-500 transition-all duration-300 relative" style={{width: `${progress}%`}}>
       ...
       <div className="w-80 bg-white/2 backdrop-blur-3xl shrink-0 flex flex-col h-full border-l border-white/5 shadow-2xl z-20">
          <div className="p-6 border-b border-white/5 flex items-center justify-between">
             ...
             {previewUrl && (
               <button onClick={() => {setImage(null); setPreviewUrl(null); setResultUrl(null); addToast("Workspace reset","info");}} className="text-xs font-bold text-gray-500 hover:text-white bg-white/5 px-3 py-1 rounded-full border border-white/5 transition-colors">Reset</button>
             )}
          </div>
          ...
          <div className="p-6 overflow-y-auto space-y-8 flex-1 custom-scrollbar">
             <div>
                ...
                <div className="grid grid-cols-3 gap-2 bg-black/40 p-1.5 rounded-2xl border border-white/5 backdrop-blur-sm">
                   {[2, 4, 8].map(s => (
                      <button key={s} onClick={() => setScale(s)} className={`py-2.5 text-sm font-bold rounded-xl transition-all ${scale === s ? 'bg-purple-600 text-white shadow-[0_0_15px_rgba(168,85,247,0.4)]' : 'text-gray-500 hover:text-gray-200 hover:bg-white/5'}`}>
                        {s}x
                      </button>
                   ))}
                </div>
             ...
             <div>
                ...
                <div className="space-y-3">
                   {["Standard", "High Fidelity", "Art & CG", "Face Recovery"].map(m => (
                      <button key={m} onClick={() => setModel(m)} className={`w-full text-left px-5 py-3.5 text-sm font-medium rounded-2xl border transition-all flex items-center gap-4 backdrop-blur-md ${model === m ? 'bg-purple-500/10 border-purple-500/50 text-purple-200 shadow-[0_0_20px_rgba(168,85,247,0.1)]' : 'bg-white/5 border-white/5 text-gray-400 hover:bg-white/10'}`}>
                        <div className={`w-3.5 h-3.5 rounded-full border-2 ${model === m ? 'border-purple-400 bg-purple-500 shadow-[0_0_10px_rgba(168,85,247,0.8)]' : 'border-gray-600 bg-transparent'}`}></div>
                        {m}
                      </button>
                   ))}
                </div>
             ...
             <div className="space-y-8 bg-black/20 p-5 rounded-3xl border border-white/5">
                <div>
                  ...
                  <input type="range" min="0" max="100" value={denoise} onChange={(e) => setDenoise(parseInt(e.target.value))} className="w-full accent-purple-500 h-1.5 bg-gray-800 rounded-lg appearance-none cursor-pointer"/>
                </div>
                <div>
                  <div className="flex justify-between mb-3">
                  ...
                  <input type="range" min="0" max="100" value={enhance} onChange={(e) => setEnhance(parseInt(e.target.value))} className="w-full accent-purple-500 h-1.5 bg-gray-800 rounded-lg appearance-none cursor-pointer"/>
                </div>
             </div>
          </div>
          ...
          <div className="p-6 border-t border-white/5 bg-black/20 backdrop-blur-md space-y-4">
             {resultUrl && (
               <div className="flex gap-3 mb-4 animate-in slide-in-from-bottom-4 duration-300">
                  <button onClick={() => setViewMode(viewMode === "result" ? "compare" : "result")} className="flex-1 py-3.5 text-xs font-bold border border-white/10 rounded-2xl hover:bg-white/10 flex items-center justify-center gap-2 backdrop-blur-md transition-all">
                     <ArrowRightLeft size={14}/> {viewMode === "compare" ? "Original" : "Compare"}
                  </button>
                  <a href={resultUrl} download={`upscaled_${scale}x.png`} onClick={()=>addToast("Upscaled image downloaded!","success")} className="flex-1 py-3.5 text-xs font-bold bg-white text-black rounded-2xl hover:bg-gray-200 flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(255,255,255,0.2)] transition-all">
                     <Download size={14}/> Save
                  </a>
               </div>
             )}
             <button onClick={processUpscale} disabled={!previewUrl || isProcessing} className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white py-4 rounded-2xl font-bold text-sm flex items-center justify-center gap-2 shadow-[0_0_30px_rgba(168,85,247,0.3)] disabled:opacity-50 disabled:grayscale transition-all relative overflow-hidden group">
     ...
     files.forEach((file, index) => {
        ...
        reader.onload = (e) => {
           ...
           img.onload = () => {
              ...
              link.href = canvas.toDataURL(format); link.click();
              if(index === files.length - 1) addToast("Conversion complete!", "success");
           };
     ...
     <div className="h-full flex flex-col items-center justify-center p-10 relative z-10">
        <div className="max-w-xl w-full text-center space-y-8 bg-white/[0.02] backdrop-blur-3xl p-12 rounded-[3rem] border border-white/10 shadow-2xl">
           ...
           <div className="p-10 border-2 border-dashed border-white/10 hover:border-blue-500 rounded-[2rem] bg-black/20 transition-all cursor-pointer relative">
              <input type="file" multiple onChange={(e) => {if(e.target.files){ setFiles(Array.from(e.target.files)); addToast(`${e.target.files.length} files queued.`,"info")}}} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"/>
              <Upload className="mx-auto mb-4 text-gray-500" size={32}/>
              <p className="text-white font-bold text-lg mb-1">Click or Drag Files</p>
              <p className="text-gray-500 text-sm">{files.length > 0 ? <span className="text-blue-400 font-bold">{files.length} files ready</span> : "Supports any image type"}</p>
           </div>
           
           <div className="flex gap-4 justify-center">
              <select onChange={(e) => setFormat(e.target.value)} className="bg-black/40 text-white px-6 py-4 rounded-2xl border border-white/10 outline-none backdrop-blur-md font-bold cursor-pointer hover:bg-white/5 transition-colors">
                 <option value="image/png">To PNG</option>
                 <option value="image/jpeg">To JPG</option>
                 <option value="image/webp">To WEBP</option>
              </select>
              <button onClick={convert} disabled={files.length === 0} className="bg-gradient-to-r from-blue-600 to-cyan-600 disabled:opacity-50 text-white px-10 py-4 rounded-2xl font-bold hover:shadow-[0_0_20px_rgba(37,99,235,0.4)] transition-all">Convert All</button>
           </div>
        </div>
     </div>
  );
...
function CalendarView() {
  const getDynamicEvents = () => {
     ...
     return baseEvents.filter(e => e.date >= new Date(today.getFullYear(), today.getMonth(), today.getDate() - 15)).slice(0, 5).map(e => ({
...
function ScraperView({ addToast }: { addToast: Function }) {
   ...
   useEffect(() => {
      setScanning(true);
      const generateDynamicTrends = async () => {
         await delay(2500); 
         const today = new Date();
         ...
         const dynamicKeywords = [
            `AI Architecture ${year}`, `Sustainable ${currentMonth}`, `Crypto Trends ${year}`,
            `Eco-friendly Packaging`, `${nextMonth} Travel Concepts`, `Remote Workspace Future`,
            `Virtual Reality UI`, `Clean Energy Revolution`
         ];
         const shuffled = dynamicKeywords.sort(() => 0.5 - Math.random()).slice(0, 4);
         const newTrends = shuffled.map(topic => {
            const vol = Math.floor(Math.random() * 500) + 50;
            let comp, status;
            if (vol > 400) { comp = "High"; status = "Saturated"; }
            else if (vol > 200) { comp = "Medium"; status = "Evergreen"; }
            else { comp = "Low"; status = "Hot Opportunity"; }
            return { topic, volume: `${vol}K/mo`, comp, status };
         });
         setTrends(newTrends); setScanning(false);
         addToast("Live market data fetched successfully", "success");
      };
      generateDynamicTrends();
   }, []);
            ...
            <div className="flex items-center gap-5 mb-10 bg-white/[0.02] p-6 rounded-3xl border border-white/5 backdrop-blur-xl">
               ...
               <div>
                  ...
                  <p className="text-gray-400 text-sm mt-1">
                    {scanning ? "Establishing secure connection to global agencies..." : `Live data stream active for ${new Date().toLocaleString('default', { month: 'long', year: 'numeric' })}`}
                  </p>
               </div>
            </div>
                     ...
                     <tbody className="divide-y divide-white/5">
                        {trends.map((t, i) => (
                           <tr key={i} className="hover:bg-white/5 transition-colors animate-in fade-in slide-in-from-bottom-8 duration-500" style={{animationDelay: `${i * 150}ms`}}>
                              ...
                              <td className="p-6 text-gray-300 font-medium">{t.comp}</td>
                              <td className="p-6">
                                 <span className={`px-4 py-2 text-xs font-bold rounded-full tracking-widest uppercase border ${t.status === 'Hot Opportunity' ? 'bg-emerald-500/10 text-emerald-300 border-emerald-500/30 shadow-[0_0_15px_rgba(16,185,129,0.2)]' : t.status === 'Evergreen' ? 'bg-blue-500/10 text-blue-300 border-blue-500/30' : 'bg-red-500/10 text-red-300 border-red-500/30'}`}>
                                    {t.status}
                                 </span>
                              </td>
                           </tr>
